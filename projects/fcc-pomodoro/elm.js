...
Elm.Constants.make = function (_elm) {
   ...
   var widgetIncrement = 5 * $Time.second;
   var initialBreakDuration = 5 * $Time.second;
   var initialSessionDuration = 30 * $Time.second;
   return _elm.Constants.values = {_op: _op
                                  ...
                                  ,widgetIncrement: widgetIncrement
                                  ,tickRate: tickRate};
};
...
Elm.Models.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var showPeriodType = function (c) {    var _p0 = c;if (_p0.ctor === "Session") {    return "Session";} else {    return "Break";}};
   var toLabel = function (time) {
      var allSeconds = $Basics.round(time / $Time.second);
      var minutes = allSeconds / 60 | 0;
      var seconds = A2($Basics._op["%"],allSeconds,60);
      return A2($Basics._op["++"],
      $Basics.toString(minutes),
      A2($Basics._op["++"],":",A2($Basics._op["++"],_U.cmp(seconds,10) < 0 ? "0" : "",$Basics.toString(seconds))));
   };
   var Break = {ctor: "Break"};
   var Session = {ctor: "Session"};
   var flipClock = function (model) {
      var _p1 = model.periodType;
      if (_p1.ctor === "Session") {
            return _U.update(model,{periodType: Break,clock: model.nextBreak,clockMax: model.nextBreak});
         } else {
            return _U.update(model,{periodType: Session,clock: model.nextSession,clockMax: model.nextSession});
         }
   };
   var Pausing = {ctor: "Pausing"};
   var Running = {ctor: "Running"};
   var Starting = {ctor: "Starting"};
   var initialModel = {mode: Starting
                      ,periodType: Session
                      ...
                      ,clock: 0
                      ,clockMax: 0};
   var AppModel = F8(function (a,b,c,d,e,f,g,h) {    return {mode: a,periodType: b,lastTick: c,clock: d,clockMax: e,nextSession: f,nextBreak: g,debug: h};});
...
Elm.Actions.make = function (_elm) {
   ...
   var ChangeNextDuration = F2(function (a,b) {    return {ctor: "ChangeNextDuration",_0: a,_1: b};});
   var TogglePause = {ctor: "TogglePause"};
   var Tick = function (a) {    return {ctor: "Tick",_0: a};};
   var NoOp = {ctor: "NoOp"};
   return _elm.Actions.values = {_op: _op,NoOp: NoOp,Tick: Tick,TogglePause: TogglePause,ChangeNextDuration: ChangeNextDuration,ToggleDebug: ToggleDebug};
};
...
Elm.Presses.make = function (_elm) {
   ...
   var handler = function (key) {
      ...
      {case "d": return $Actions.ToggleDebug;
         case "D": return $Actions.ToggleDebug;
         case " ": return $Actions.TogglePause;
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "Tick": var _p2 = _p0._0;
           var model$ = _U.update(model,{lastTick: _p2});
           var _p1 = model.mode;
           switch (_p1.ctor)
           {case "Running": var newclock = model.clock - (_p2 - model.lastTick);
                return _U.cmp(newclock,0) > 0 ? _U.update(model$,{clock: newclock}) : $Models.flipClock(model$);
              case "Pausing": return model$;
              default: return _U.update(model$,{mode: $Models.Pausing,periodType: $Models.Session,clock: model.nextSession,clockMax: model.nextSession});}
         case "TogglePause": var _p3 = model.mode;
           switch (_p3.ctor)
           {case "Running": return _U.update(model,{mode: $Models.Pausing});
              case "Pausing": return _U.update(model,{mode: $Models.Running});
              default: return model;}
         case "ChangeNextDuration": var _p8 = _p0._0;
           var _p7 = _p0._1;
           if (_U.eq(_p8,model.periodType)) {
                 var _p4 = model.mode;
                 switch (_p4.ctor)
                 {case "Running": return model;
                    case "Starting": return model;
                    default: var _p5 = _p8;
                      if (_p5.ctor === "Session") {
                            var clockUsed = model.clockMax - model.clock;
                            var nextSession$ = A2($Basics.max,0,model.nextSession + _p7);
                            return _U.update(model,{nextSession: nextSession$,clockMax: nextSession$,clock: A2($Basics.max,0,nextSession$ - clockUsed)});
                         } else {
                            var clockUsed = model.clockMax - model.clock;
                            var nextBreak$ = A2($Basics.max,0,model.nextBreak + _p7);
                            return _U.update(model,{nextBreak: nextBreak$,clockMax: nextBreak$,clock: A2($Basics.max,0,nextBreak$ - clockUsed)});
                         }}
              } else {
                 var _p6 = _p8;
                 if (_p6.ctor === "Session") {
                       return _U.update(model,{nextSession: A2($Basics.max,0,model.nextSession + _p7)});
                    } else {
                       return _U.update(model,{nextBreak: A2($Basics.max,0,model.nextBreak + _p7)});
                    }
              }
         default: return _U.update(model,{debug: $Basics.not(model.debug)});}
   });
...
Elm.View.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var tomato = F2(function (address,model) {
      var clipWidth = 220;
      var clipX = 0;
      var tomatoFraction = _U.eq(model.clockMax,0) ? 0 : 1 - model.clock / model.clockMax;
      var clipY = 200 - 180 * tomatoFraction;
      var clipHeight = 180 * tomatoFraction;
      return A2($Html.div,
      _U.list([A2($Html$Events.onClick,address,$Actions.TogglePause)]),
      _U.list([A2($Svg.svg,
      _U.list([$Svg$Attributes.width("220"),$Svg$Attributes.height("220")]),
      _U.list([A2($Svg.circle,
              _U.list([$Svg$Attributes.$class("tomato-outer-circle"),$Svg$Attributes.cx("110"),$Svg$Attributes.cy("110"),$Svg$Attributes.r("100")]),
              _U.list([]))
              ,A2($Svg.defs,
              _U.list([]),
              _U.list([A2($Svg.clipPath,
              _U.list([$Svg$Attributes.id("tomato-inner-circle-clip")]),
              _U.list([A2($Svg.rect,
              _U.list([$Svg$Attributes.x($Basics.toString(clipX))
                      ,$Svg$Attributes.y($Basics.toString(clipY))
                      ,$Svg$Attributes.width($Basics.toString(clipWidth))
                      ,$Svg$Attributes.height($Basics.toString(clipHeight))]),
              _U.list([]))]))]))
              ,A2($Svg.circle,
              _U.list([$Svg$Attributes.$class(A2($Basics._op["++"],"tomato-inner-circle-",$Models.showPeriodType(model.periodType)))
                      ,$Svg$Attributes.clipPath("url(#tomato-inner-circle-clip)")
                      ,$Svg$Attributes.cx("110")
                      ,$Svg$Attributes.cy("110")
                      ,$Svg$Attributes.r("90")]),
              _U.list([]))
              ,A2($Svg.text$,
              _U.list([$Svg$Attributes.$class("tomato-label"),$Svg$Attributes.x("110"),$Svg$Attributes.y("60"),$Svg$Attributes.textAnchor("middle")]),
              _U.list([$Svg.text($Models.showPeriodType(model.periodType))]))
              ,A2($Svg.text$,
              _U.list([$Svg$Attributes.$class("tomato-timer"),$Svg$Attributes.x("110"),$Svg$Attributes.y("160"),$Svg$Attributes.textAnchor("middle")]),
              _U.list([$Svg.text($Models.toLabel(model.clock))]))]))]));
   });
   var control = F4(function (address,label,periodType,time) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("control")]),
      _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("control-label ")]),_U.list([$Html.text(label)]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("control-widget")]),
              _U.list([A2($Html.button,
                      _U.list([$Html$Attributes.$class("widget-button")
                              ,A2($Html$Events.onClick,address,A2($Actions.ChangeNextDuration,periodType,0 - $Constants.widgetIncrement))]),
                      _U.list([A2($Html.span,_U.list([$Html$Attributes.$class("fa fa-minus-circle fa-lg")]),_U.list([]))]))
                      ,$Html.text($Models.toLabel(time))
                      ,A2($Html.button,
                      _U.list([$Html$Attributes.$class("widget-button")
                              ,A2($Html$Events.onClick,address,A2($Actions.ChangeNextDuration,periodType,$Constants.widgetIncrement))]),
                      _U.list([A2($Html.span,_U.list([$Html$Attributes.$class("fa fa-plus-circle fa-lg")]),_U.list([]))]))]))]));
   });
   var controlsBar = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("controls-bar")]),
      _U.list([A4(control,address,"BREAK LENGTH",$Models.Break,model.nextBreak),A4(control,address,"SESSION LENGTH",$Models.Session,model.nextSession)]));
   });
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.div,
              ...
              _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("title")]),_U.list([$Html.text("FreeCodeCamp")]))
                      ,A2(controlsBar,address,model)
                      ,A2($Html.div,_U.list([$Html$Attributes.$class("tomato-container")]),_U.list([A2(tomato,address,model)]))]))
              ,model.debug ? A2($Html.div,_U.list([$Html$Attributes.$class("debug")]),_U.list([$Html.text($Basics.toString(model))])) : A2($Html.div,
              _U.list([]),
              _U.list([]))]));
   });
   return _elm.View.values = {_op: _op,view: view,controlsBar: controlsBar,control: control,tomato: tomato};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var app = $StartApp.start({init: {ctor: "_Tuple2",_0: $Models.initialModel,_1: $Effects.none}
                             ,inputs: _U.list([A2($Signal.map,$Presses.handler,$Keyboard.presses)
                                              ,A2($Signal.map,$Actions.Tick,$Time.every($Constants.tickRate))])
